...
def train_epoch(model, dataloader, optimizer, criterion, le):
    ...
    # Accumulate all predictions and true labels of the epoch to calculate metrics
    # This is necessary to calculate correct metrics when batches have different sizes
    all_predictions = []
    all_labels = []
    ...
    # Calculate metrics over the entire epoch set
...
def evaluate_model(model, dataloader, criterion, le):
    model.eval()  # Evaluation mode (deactivates dropout, etc.)
    loss_sum = 0.0
